...
(function($) {
  ...
  var root = this,
      previousWildchatUI = root.WildchatUI;

  root.WildchatUI = WildchatUI;

  if (!self.WildchatDefaultTemplates) {
    throw new Error("Unable to find chat templates!");
  }

  function WildchatUI(wilddogRef, el, options) {
    var self = this;

    if (!wilddogRef) {
      throw new Error('WildchatUI: Missing required argument `wilddogRef`');
    }

    if (!el) {
      throw new Error('WildchatUI: Missing required argument `el`');
    }
    ...
    this._el = el;
    this._user = null;
    this._chat = new Wildchat(wilddogRef, options);
    ...
    this.$wrapper = $('#wildchat');
    this.$roomList = $('#wildchat-room-list');
    this.$tabList = $('#wildchat-tab-list');
    this.$tabContent = $('#wildchat-tab-content');
    this.$messages = {};
  ...
  // Run WildchatUI in *noConflict* mode, returning the `WildchatUI` variable to
  // its previous owner, and returning a reference to the WildchatUI object.
  WildchatUI.noConflict = function noConflict() {
    root.WildchatUI = previousWildchatUI;
    return WildchatUI;
  };

  WildchatUI.prototype = {
    ...
    _renderLayout: function() {
      var template = WildchatDefaultTemplates["templates/layout-full.html"];
    ...
    _onUpdateUser: function(user) {
      ...
      $('[data-event="wildchat-user-mute-toggle"]').each(function(i, el) {
    ...
    _onChatInvite: function(invitation) {
      var self = this;
      var template = WildchatDefaultTemplates["templates/prompt-invitation.html"];
    ...
    _onChatInviteResponse: function(invitation) {
      ...
      var self = this,
          template = WildchatDefaultTemplates["templates/prompt-invite-reply.html"],
          $prompt;
   ...
   */
  WildchatUI.prototype.setUser = function(userId, userName) {
    var self = this;
   ...
   */
  WildchatUI.prototype.on = function(eventType, cb) {
    var self = this;
   ...
   */
  WildchatUI.prototype._bindSuperuserUIEvents = function() {
    var self = this,
        ...
        clearMessageContextMenus = function() {
          ...
          $('[data-toggle="wildchat-contextmenu"]').each(function() {
            $(this).remove();
          });

          // Remove any messages currently highlighted.
          $('#wildchat .message.highlighted').each(function() {
        ...
        showMessageContextMenu = function(event) {
          var $this = $(this),
              ...
              template = WildchatDefaultTemplates["templates/message-context-menu.html"],
    ...
    $(document).delegate('[data-class="wildchat-message"]', 'contextmenu', showMessageContextMenu);

    // Handle click of the 'Warn User' contextmenu item.
    $(document).delegate('[data-event="wildchat-user-warn"]', 'click', function(event) {
   ...
   */
  WildchatUI.prototype._bindForHeightChange = function() {
    var self = this,
        $el = $(this._el),
        lastHeight = null;
   ...
   */
  WildchatUI.prototype._bindForTabControls = function() {
    var self = this;

    // Handle click of tab close button.
    $(document).delegate('[data-event="wildchat-close-tab"]', 'click', function(event) {
   ...
   */
  WildchatUI.prototype._bindForRoomList = function() {
    var self = this;

    $('#wildchat-btn-rooms').bind('click', function() {
      if ($(this).parent().hasClass('open')) {
        return;
      }

      var $this = $(this),
          template = WildchatDefaultTemplates["templates/room-list-item.html"],
   ...
   */
  WildchatUI.prototype._bindForUserRoomList = function() {
    var self = this;

    // Upon click of the dropdown, autofocus the input field and trigger list population.
    $(document).delegate('[data-event="wildchat-user-room-list-btn"]', 'click', function(event) {
      event.stopPropagation();

      var $this = $(this),
          roomId = $this.closest('[data-room-id]').data('room-id'),
          template = WildchatDefaultTemplates["templates/room-user-list-item.html"],
   ...
   */
  WildchatUI.prototype._bindForUserSearch = function() {
    var self = this,
        ...
        userSearch = function(targetId, templateId, controlsId, prefix, startAt, endAt) {
          var $target = $('#' + targetId),
              $controls = $('#' + controlsId),
              template = WildchatDefaultTemplates[templateId];
          ...
          self._chat.getUsersByPrefix(prefix, startAt, endAt, self.maxUserSearchResults, function(users) {
            ...
            if ($controls) {
              $prevBtn = $controls.find('[data-toggle="wildchat-pagination-prev"]');
              $nextBtn = $controls.find('[data-toggle="wildchat-pagination-next"]');

              // Sort out configuration for the 'next' button
              if (lastResult) {
                $nextBtn
                  .data('event', 'wildchat-user-search')
    ...
    $(document).delegate('[data-event="wildchat-user-search"]', 'keyup', handleUserSearchSubmit);
    $(document).delegate('[data-event="wildchat-user-search"]', 'click', handleUserSearchSubmit);

    // Upon click of the dropdown, autofocus the input field and trigger list population.
    $(document).delegate('[data-event="wildchat-user-search-btn"]', 'click', function(event) {
      event.stopPropagation();
      var $input = $(this).next('div.wildchat-dropdown-menu').find('input');
      $input.focus();
      $input.trigger(jQuery.Event('keyup'));
    });

    // Ensure that the dropdown stays open despite clicking on the input element.
    $(document).delegate('[data-event="wildchat-user-search"]', 'click', function(event) {
      event.stopPropagation();
    });
  };
   ...
   */
  WildchatUI.prototype._bindForUserMuting = function() {
    var self = this;
    $(document).delegate('[data-event="wildchat-user-mute-toggle"]', 'click', function(event) {
      var $this = $(this),
          ...
          template = WildchatDefaultTemplates["templates/prompt-user-mute.html"];
  ...
  /**
   * Binds to elements with the data-event='wildchat-user-(private)-invite' and
   * handles invitations as well as room creation and entering.
   */
  WildchatUI.prototype._bindForChatInvites = function() {
    var self = this,
        renderInvitePrompt = function(event) {
          var $this = $(this),
              ...
              roomId = $this.closest('[data-room-id]').data('room-id'),
              userName = $this.closest('[data-user-name]').data('user-name'),
              template = WildchatDefaultTemplates["templates/prompt-invite-private.html"],
              $prompt;
    ...
    $(document).delegate('[data-event="wildchat-user-chat"]', 'click', renderPrivateInvitePrompt);
    $(document).delegate('[data-event="wildchat-user-invite"]', 'click', renderInvitePrompt);
  };

  /**
   * Binds to room dropdown button, menu items, and create room button.
   */
  WildchatUI.prototype._bindForRoomListing = function() {
    var self = this,
        $createRoomPromptButton = $('#wildchat-btn-create-room-prompt'),
        $createRoomButton = $('#wildchat-btn-create-room'),
        renderRoomList = function(event) {
          var type = $(this).data('room-type');

          self.sortListLexicographically('#wildchat-room-list');
        };
    ...
    $createRoomButton.bind('click', function(event) {
      var roomName = $('#wildchat-input-room-name').val();
      $('#wildchat-prompt-create-room').remove();
   ...
   */
  WildchatUI.prototype._setupTabs = function() {
    var self = this,
        show = function($el) {
          var $this = $el,
              $ul = $this.closest('ul:not(.wildchat-dropdown-menu)'),
        ...
        activate = function (element, container, callback) {
          ...
          function next() {
            $active
              .removeClass('active')
              .find('> .wildchat-dropdown-menu > .active')
              .removeClass('active');
            ...
            if (element.parent('.wildchat-dropdown-menu')) {
              element.closest('li.wildchat-dropdown').addClass('active');
            }
    ...
    $(document).delegate('[data-toggle="wildchat-tab"]', 'click', function(event) {
      event.preventDefault();
      show($(this));
    });
  };
   ...
   */
  WildchatUI.prototype._setupDropdowns = function() {
    var self = this,
        toggle = '[data-toggle=wildchat-dropdown]',
        ...
        clearMenus = function() {
          $('[data-toggle=wildchat-dropdown]').each(function() {
      ...
      $(document)
        ...
        .delegate('.wildchat-dropdown-menu', 'click', function(event) { event.stopPropagation(); })
        .delegate('[data-toggle=wildchat-dropdown]', 'click', toggleDropdown);
  };
   ...
   */
  WildchatUI.prototype._bindTextInputFieldLimits = function() {
   ...
   */
  WildchatUI.prototype.renderAlertPrompt = function(title, message) {
    var template = WildchatDefaultTemplates["templates/prompt-alert.html"],
   ...
   */
  WildchatUI.prototype.toggleInputs = function(isEnabled) {
    $('#wildchat-tab-content textarea').each(function() {
      var $this = $(this);
      if (isEnabled) {
        $(this).val('');
      } else {
    ...
    $('#wildchat-input-name').prop('disabled', !isEnabled);
  };
   ...
   */
  WildchatUI.prototype.attachTab = function(roomId, roomName) {
    var self = this;
    ...
    var tabTemplate = WildchatDefaultTemplates["templates/tab-content.html"];
    var $tabContent = $(tabTemplate(room));
    this.$tabContent.prepend($tabContent);
    var $messages = $('#wildchat-messages' + roomId);
    ...
    var tabListTemplate = WildchatDefaultTemplates["templates/tab-menu-item.html"];
    ...
    var tabWidth = Math.floor($('#wildchat-tab-list').width() / tabs.length);
    ...
    $('#wildchat-btn-room-user-list-' + roomId).bind('click', function() {
      self.sortListLexicographically('#wildchat-room-user-list-' + roomId);
      return false;
    });
   ...
   * @param    {string}    roomId
   */
  WildchatUI.prototype.focusTab = function(roomId) {
    if (this.$messages[roomId]) {
   ...
   * @param    {string}    roomId
   */
  WildchatUI.prototype.removeTab = function(roomId) {
    ...
    this.$tabList.find('[data-toggle="wildchat-tab"]').first().trigger('click');
   ...
   * @param    {string}    message
   */
  WildchatUI.prototype.showMessage = function(roomId, rawMessage) {
    var self = this;
    ...
    var template = WildchatDefaultTemplates["templates/message.html"];
   ...
   */
  WildchatUI.prototype.removeMessage = function(roomId, messageId) {
   ...
   */
  WildchatUI.prototype.sortListLexicographically = function(selector) {
   ...
   * @return   {string}
   */
  WildchatUI.prototype.trimWithEllipsis = function(str, length) {
   ...
   * @param    {string}    date
   */
  WildchatUI.prototype.formatTime = function(timestamp) {
   ...
   */
  WildchatUI.prototype.promptCreateRoom = function() {
    var self = this;
    var template = WildchatDefaultTemplates["templates/prompt-create-room.html"];
    ...
    $prompt.find('[data-toggle=submit]').first().click(function() {
      var name = $prompt.find('[data-input=wildchat-room-name]').first().val();
      if (name !== '') {
    ...
    $prompt.find('[data-input=wildchat-room-name]').first().focus();
    $prompt.find('[data-input=wildchat-room-name]').first().bind('keydown', function(e) {
      if (e.which === 13) {
        var name = $prompt.find('[data-input=wildchat-room-name]').first().val();
        if (name !== '') {
          self._chat.createRoom(name, 'public');
   ...
   */
  WildchatUI.prototype.prompt = function(title, content) {
    var template = WildchatDefaultTemplates["templates/prompt.html"],
        $prompt;
  ...
  WildchatUI.prototype.linkify = function(str) {
    var self = this;
    return str
